# クロージャの基本 (closure-basic)

## 概要

このプロジェクトは Rust Tech の記事  
[【Rust入門】クロージャの基本と使い方を分かりやすく解説](https://rust-tech.nkhn37.net/rust-closure-basic/)
で紹介しているサンプルコードです。
...
`-p closure-basic` を指定して `cargo run` を実行します。

```bash
cargo run -p closure-basic
```

出力例：

```
cargo run -p closure-basic --example closure_basic
cargo run -p closure-basic --example closure_capture
```
